...
def stage_3_referee(ctx: Ctx) -> StageOutcome:
    ...
    # Fail closed: always request a report and grounding dir. Do not probe the
    # sibling script for flag support — that fail-opened on origin/main.
    quote_argv.extend(["--grounding-dir", str(ctx.harness_dir / "grounding")])
    quote_argv.extend(["--report", str(report_path)])
    clean_ndjson = ctx.harness_dir / "clean.ndjson"
    if clean_ndjson.is_file():
        quote_argv.extend(["--target", str(clean_ndjson)])
    qproc = ctx.runner.run(quote_argv)
    tail = (tail + "\n" + record_tool(ctx, qproc)).strip()

    if report_path.is_file():
        payload = json.loads(report_path.read_text(encoding="utf-8"))
        if isinstance(payload, dict) and payload.get("needs_grounding") is not None:
            blocked = list(payload["needs_grounding"])
        else:
            rows = payload if isinstance(payload, list) else (
                payload.get("sources") or payload.get("rows") or []
            )
            blocked = [
                r for r in rows
                if isinstance(r, dict) and r.get("status") in ("unreachable", "blocked")
            ]
        if blocked:
    ...
    return Pause(
        ...
        msg="quotes not confirmed (no report or verifier error) — grounding required",
        stdout_tail=tail,
    )
