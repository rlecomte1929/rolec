...
class SubprocessToolRunner:
    ...
    def supports(self, script_name: str, flag: str) -> bool:
        return quote_script_supports(SCRIPTS / script_name, flag)
...
def quote_script_supports(script: Path, flag: str) -> bool:
    """Filesystem probe used by the real runner. Absent script ⇒ True (flags are still requested)."""
...
def stage_3_referee(ctx: Ctx) -> StageOutcome:
    ...
    if ctx.runner.supports("verify_batch_quotes.py", "--grounding-dir"):
        quote_argv.extend(["--grounding-dir", str(ctx.harness_dir / "grounding")])
    if ctx.runner.supports("verify_batch_quotes.py", "--report"):
