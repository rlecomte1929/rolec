...
def _pick_nationality(rec: dict, applies: dict) -> Optional[str]:
    ...
    for raw in candidates:
        mapped = NATIONALITY_ALIASES.get(str(raw), str(raw) if str(raw) in NATIONALITY_CLASSES else None)
        if mapped in NATIONALITY_CLASSES:
            return mapped
    return None
...
def _confidence(rec: dict) -> Tuple[str, float]:
    ...
    if isinstance(score, (int, float)) and 0 < float(score) <= 1:
        for lab, val in SCORE.items():
            if abs(float(score) - val) < 1e-9:
                return lab, float(val)
        # Non-canonical but valid score: keep medium label only if we must pair.
        # Pairing requires confidence_score == SCORE[confidence], so snap to nearest.
        nearest = min(SCORE.items(), key=lambda kv: abs(kv[1] - float(score)))
...
def flatten_profile(rec: dict, profile: str) -> dict:
    ...
    if profile == "requirement_items":
        ...
        if not out.get("fact_key"):
            # Beam batches often omit fact_key; topic+source is still required, but a
            # fact_key is required by parsers — fail loudly rather than invent a slug.
            if rec.get("fact_uid"):
                out["fact_key"] = rec["fact_uid"]
    return out
...
def assert_paired(out: dict) -> None:
    ...
    if score == 0 or score is None:
        raise ConvertError(f"{out.get('fact_key')}: confidence_score must not be 0/None")
...
def convert_record(
    ...
    *,
    ...
    if status not in STATUS_TO_PURPOSE:
        # Required, but not in the closed set — still stamp what the artifact/operator gave;
        # STATUS_TO_PURPOSE is downstream mapping, not a reject list beyond being present.
        pass
    ...
    if nat is None:
        ...
        report["missing_nationality"].append(_dedupe_key({**src, "destination_country": dest, "entity_topic_key": topic, "fact_key": fact_key}))
    else:
        applies["nationality"] = nat

    if src.get("pillar") is not None:
        applies["pillar"] = src["pillar"]
    elif "pillar" in applies:
        pass  # already there, untouched
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--default-status", default=None)
