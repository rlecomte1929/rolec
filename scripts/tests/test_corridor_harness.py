...
def test_referee_pause_unreachable_when_verifier_exists_without_report(tmp_path, monkeypatch):
    """Regression: origin/main's verify_batch_quotes.py exists but has no --report.

    Stage 3 must ask the injected runner (FakeToolRunner.supports → True), not probe
    the on-disk script, or the unreachable row never lands on the pause worklist.
    """
