...
def test_happy_path_dry_no_stage(tmp_path):
    ...
    for call in _script_calls(runner, "import_otto_facts.py"):
...
def test_convert_gates_when_not_parsers_vocab(tmp_path):
    rec = {"topic": "not-vocab", "text": "nope"}
    batch = _batch(tmp_path, [rec])
    # rewrite facts without vocab keys; keep shape A counts
