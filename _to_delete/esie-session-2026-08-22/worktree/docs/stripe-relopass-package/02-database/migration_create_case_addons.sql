-- Migration: create_case_addons
-- ReloPass — Stripe Integration v1.0 (stub — not used until v1)
-- 2026-07-18
...
CREATE TABLE IF NOT EXISTS case_addons (
  ...
  addon_type               TEXT NOT NULL CHECK (addon_type IN (
                             'vendor_movers', 'vendor_immigration',
                             'vendor_tax', 'vendor_schools', 'vendor_pets'
                           )),
...
CREATE INDEX IF NOT EXISTS idx_case_addons_case_id
  ON case_addons(case_id);
