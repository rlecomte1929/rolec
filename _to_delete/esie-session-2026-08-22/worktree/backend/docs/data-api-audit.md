...
## FRIDAY-003c — production deploy prep + live verification (2026-06-10)

*Generated by Claude Code preparing FRIDAY-003c (AIQ-764). Deploy NOT yet executed — see hand-off below.*

### Live-grant verification (prod `nsvefcvpvwwwhuqyuqmp`)

Before deploying, the 24 target `(table, role)` pairs in
`20260605200000_data_api_opt_out.sql` were checked against the **live prod**
grant state (`information_schema.role_table_grants`):

> **All 24 targeted grants are already absent.** Every table exists; not one of
> the `anon`/`authenticated` grants the migration revokes is currently present.

This means the desired end-state (no Data API grants on these 24 tables) is
**already satisfied on prod** — almost certainly a side-effect of the SEC-002 RLS
hardening, after which `REVOKE ALL … FROM anon` became a standard new-table gate.

**Implication:** applying this migration to prod is a **verified no-op on the grant
state** — it revokes nothing because nothing is granted. Its remaining value is
*codifying* the intended state in migration history (SOC-2 traceability) and the
`to_regclass`-guarded REVOKEs as enforce-as-code going forward.

- Ledger state: `20260605200000` is **NOT** in `supabase_migrations.schema_migrations` (genuinely pending).
- Risk: ~zero. The earlier concern (over-restricting → breaking the frontend's
  direct supabase-js calls) does not apply — there is nothing to revoke.
- The **24h smoke-test window is unnecessary** for this migration: it changes no
  live grant, and pre-launch there is no real traffic to smoke-test against.

### Verification test fixed

`backend/tests/test_data_api_opt_out.py` static mode was asserting literal
`REVOKE … FROM <role>;` statements, but FRIDAY-003b rewrote the migration into a
`to_regclass`-guarded `do`-block driven by a `(tbl, role, priv)` VALUES list — so
the static tests were failing (and the KEEP/VERIFY guards passed vacuously). The
tests now parse the VALUES list directly: **36 passed**, and the KEEP/VERIFY
membership checks are meaningful again.

### Hand-off — how to deploy (Romain)

Because it's a verified no-op, no low-traffic window or 24h smoke is required. To
codify it + reconcile the ledger (per CLAUDE.md "Ledger reconciliation"):

1. Apply the migration body to prod (it's idempotent + a no-op on current grants):
   run the `do $$ … $$;` block from `supabase/migrations/20260605200000_data_api_opt_out.sql`
   via `execute_sql` (no schema change → not an `apply_migration`).
2. Record it in the ledger so the drift check stays green:
   `INSERT INTO supabase_migrations.schema_migrations (version, name) VALUES ('20260605200000', 'data_api_opt_out');`
3. Re-run the live verification (the 24-pair query above, or
   `RELOPASS_TEST_DB_URL=<prod> pytest backend/tests/test_data_api_opt_out.py`) and
   confirm all 24 pairs = 0 grants, KEEP tables retain grants.
4. Fill in the deploy record below.

> _Deployed to production: __________ (date) by __________. Live verification: all 24
> target pairs 0 grants ☐ · KEEP tables retain grants ☐ · ledger row present ☐._
