...
3. **Verify the manifest's claims against the live schema before writing anything.** A
   manifest names a target table and key; that is a claim. It has twice been wrong — IE→ES
   named a table that could not hold the data, and B3 named four tables
   (`kg_corridors`, `kg_corridor_requirements`, `kg_employee_types`, `geo_city_content`) that
   **do not exist in this repo at all**, alongside a `tools/wave2-import-pipeline.mjs` that
   exists on no ref. Confirm the table, then map the fields.
