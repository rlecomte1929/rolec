...
| 8 | 2026-05-26 | `audit/stage-8-arch-survey` | (pending) | **B6 (route-auth CI)** verified closed on main: scripts/check_route_auth.py (AST analysis) + 88-entry allowlist + ci.yml job wired. **C2 (6:61 migration)** substantially advanced: 27 routers in modular layer (was 6); MIGRATION_PLAN.md (404 LOC) on main; Month-1 moved 21 auth+employee+HR routers. **B7 (query-count middleware)** newly built: ASGI middleware + SQLAlchemy listener + 4 tests; WARN above 10 queries/request flags N+1. **8c bundle** surveyed (14 MB dist, 423 kB main, vendor split + lazy-loading working). **8e/f/g (cases.py / immigration.py / database.py decompositions)** filed as multi-week followups. | [`audit/re-audit-stage-8-arch.md`](re-audit-stage-8-arch.md) | Eng manager: **6.0 → 7.5** (+1.5); Performance: **6.0 → 7.0** (+1.0) | 2 new files + 1 edit, ~180 LOC. 59/59 tests pass (4 new + 55 P1). |
| 7 | 2026-05-26 | `audit/stage-7-llm-hardening` | (pending) | **B5 (llm_client wrapper)** verified closed on main: 363 LOC, timeout + retry + structured-output schemas (OpenAI + Anthropic) + structured logging. **SEC-5 (OCR)** closed: passport extractor uses wrapper with timeout=45, max_retries=2, json_object mode. **SEC-6 (assistant_router)** closed: enterprise-grade 4-stage defense-in-depth (topic_classifier → retrieve_policy → input_guardrails → output_guardrails with cross-tier fence + faithfulness check); hardcoded fallbacks; temperature=0; audit-log without raw queries. **Residual** 7 direct LLM call sites bypass wrapper → AUDIT-B5-followup. | [`audit/re-audit-stage-7-llm-hardening.md`](re-audit-stage-7-llm-hardening.md) | Security: **7.5 → 8.5** (+1.0) | No source-code edits. Adversarial read found no exploit paths across the reviewed surfaces. |
| 6 | 2026-05-26 | `audit/stage-6-design-system` | [#127](https://github.com/rlecomte1929/rolec/pull/127) | **B3 (empty-state pass)** closed for top-traffic pages: Dashboard + HrCommandCenter patched per docs/product-copy-rules.md; AdminProspects already compliant; EmployeeJourney verified in-context (sibling pending-invitations list handles actionable path). **B2 (raw HTML migration)** acknowledged-not-closed: Phase 2 named 37 sites; current count is **174** (121 buttons + 53 inputs) — mostly platform-v2/ growth since Phase 2. Filed as AUDIT-B2-followup (P2, 4-phase decomposition). | [`audit/re-audit-stage-6-design-system.md`](re-audit-stage-6-design-system.md) | Design (live): **7.3 → 7.6** (+0.3) | 2 source-code edits, ~10 LOC. Honest scope decision: 174-site refactor is multi-week, not one-stage. |
...
| 9 | (deferred) | — | — | Customer-discovery wave — Stage 9 deferred by user choice. Remains as the program's biggest open strategic gap (CEO-1 from v1 synthesis). Tracked as [AUDIT-CUSTDISC](https://www.notion.so/36b887c64d4881f19656dc8b4bd68ff7). | n/a | (would have moved CEO confidence 75% → 85%+) | Non-code stage. The single highest-leverage non-engineering action still open. |
| 10 | 2026-05-26 | `audit/stage-10-synthesis-v2` | (pending) | **Program closure.** [`audit/00-synthesis-v2.md`](00-synthesis-v2.md) supersedes v1 with side-by-side scorecard, full findings closure status (Tier A: 9/9 closed; Tier B: 7/10 + 2 filed + 1 deferred; Tier C: 2/10 + 8 deferred), 11 open follow-ups in Notion, 4 closed during the program, lessons learned, recommended 30/60/90-day next steps. | [`audit/00-synthesis-v2.md`](00-synthesis-v2.md) | Composite: **~6.0 → ~7.4** (+1.4 across 11 lenses) | The 11-stage program closes here. The team has the runway list for the next quarter. |
...
| B5 | `llm_client.py` wrapper | ENG-7 / SEC-5 / P1-6 | [AUDIT-B5](https://www.notion.so/36b887c64d4881cbbe0ffb7a89863224) | Stage 7 (wrapper + OCR + assistant closed; 7 sites residual → [B5-followup](https://www.notion.so/36c887c64d4881279ac0e0fd2c46dc73)) | — |
| B6 | Route-auth CI check | ENG-2 | [AUDIT-B6](https://www.notion.so/36b887c64d488193b15dca3faca08067) | Stage 8 (closed; verified) | — |
| B7 | Per-request query-count middleware | PERF-5 | [AUDIT-B7](https://www.notion.so/36b887c64d488125b03bd286e279ef0e) | Stage 8 (closed; built this stage) | — |
