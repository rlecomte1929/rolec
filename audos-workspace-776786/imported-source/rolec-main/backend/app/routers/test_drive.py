...
class ProvisionRequest(BaseModel):
    ...
    # TD-SEG (ICP instrumentation): two OPTIONAL one-tap segmentation answers from the
    # /test-drive start form — company size and relocation ownership. NULL = skipped;
    # skipping must never block provisioning. Stored on the test_sessions row and later
    # stamped onto the linked survey_responses row so segment + survey are queryable
    # together (does "trust = yes" cluster in 50-500 or 2000+?).
    company_size: Optional[str] = Field(None, pattern="^(<50|50-500|500-2000|2000\\+)$")
    owns_relocation: Optional[str] = Field(None, pattern="^(run_it|touch_it|not_my_area)$")
    ...
    with db.engine.begin() as conn:
        conn.execute(
            text(
                ...
                f"(id, campaign, corridor_id, tester_segment, company_size, owns_relocation, "
                f"first_name_label, tester_name, tester_email, hr_username, emp_username, status) "
                f"VALUES ({id_expr}, :campaign, :corridor_id, :tester_segment, :company_size, "
                f":owns_relocation, :first_name_label, :tester_name, :tester_email, "
            ...
            ),
            {
                ...
                # TD-SEG: optional one-tap segmentation — binds NULL when skipped.
                "company_size": body.company_size,
                "owns_relocation": body.owns_relocation,
            },
        )
...
_SURVEY_COLUMNS = (
    ...
    "permit_relevance "  # TD-EEA: EEA free movement vs Employment Permit relevance
...
class SurveyRequest(BaseModel):
    ...
    # TD-EEA (INSEAD cohort): does the tester face Employment Permit moves (sponsoring a
    # work permit / cross-border employment) vs lighter EEA free-movement admin? One tap.
    permit_relevance: Optional[str] = Field(None, pattern="^(yes|not_yet|no)$")
...
def _session_context(session_id: Optional[str]) -> Dict[str, Any]:
    ...
    for which campaign + corridor this tester actually ran. TD-SEG: the provision-time
    segmentation (company_size / owns_relocation) rides along the same lookup so the
    survey write can stamp it onto the survey_responses row. Returns a dict with keys
    campaign / corridor_id / company_size / owns_relocation; every value is None when
    there's no session or the lookup fails (caller falls back to body/default).
    Best-effort — never raises into the survey write."""
    empty: Dict[str, Any] = {
        "campaign": None, "corridor_id": None, "company_size": None, "owns_relocation": None,
    }
    if not session_id:
        return empty
    try:
        ...
        with db.engine.connect() as conn:
            row = conn.execute(
                text(
                    f"SELECT campaign, corridor_id, company_size, owns_relocation "
                    f"FROM test_sessions WHERE id = {sid_expr}"
                ),
                {"sid": session_id},
            ).mappings().first()
        if row:
            return {k: row.get(k) for k in empty}
...
def survey(body: SurveyRequest, request: Request):
    ...
    sess = _session_context(sid)
    campaign = sess["campaign"] or (body.campaign or "").strip() or os.getenv("RELOPASS_TEST_DRIVE_CAMPAIGN", "insead-2026")
    corridor_id = sess["corridor_id"] or body.corridor_id
    ...
    # TD-SEG: stamp the provision-time segmentation onto the survey row, derived from the
    # linked session (exactly like campaign/corridor above) — segment + survey answers
    # then sit on ONE queryable survey_responses row, no join needed.
    params["company_size"] = sess["company_size"]
    params["owns_relocation"] = sess["owns_relocation"]

    col_sql = ", ".join(
        ["id", "session_id", "company_size", "owns_relocation"]
        + [c for c in _SURVEY_COLUMNS if c != "session_id"]
    )
    val_sql = ", ".join(
        [id_expr, session_expr, ":company_size", ":owns_relocation"]
