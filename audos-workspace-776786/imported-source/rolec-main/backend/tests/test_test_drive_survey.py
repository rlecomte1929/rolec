...
class TestTestDriveSurvey(unittest.TestCase):
    ...
    def test_permit_relevance_persists(self):
        """TD-EEA: permit_relevance (yes|not_yet|no) is written to the survey_responses INSERT."""
        db = MagicMock()
        db.engine.begin.return_value.__enter__.return_value.execute.return_value.first.return_value = None
        with patch.dict(os.environ, _ENABLED, clear=False), \
                patch("backend.app.routers.test_drive.db", db):
            resp = self.client.post(
                "/api/test-drive/survey", json=_body(permit_relevance="not_yet"),
            )
        self.assertEqual(resp.status_code, 200, resp.text)
        conn = db.engine.begin.return_value.__enter__.return_value
        survey_calls = [c for c in conn.execute.call_args_list if "INSERT INTO survey_responses" in str(c.args[0])]
        self.assertEqual(len(survey_calls), 1)
        self.assertIn("permit_relevance", str(survey_calls[0].args[0]))
        self.assertEqual(survey_calls[0].args[1]["permit_relevance"], "not_yet")

    def test_bad_permit_relevance_returns_422(self):
        """TD-EEA: the enum (yes|not_yet|no) is validated at the API — garbage is rejected."""
        with patch.dict(os.environ, _ENABLED, clear=False):
            resp = self.client.post("/api/test-drive/survey", json=_body(permit_relevance="perhaps"))
    ...
    def test_segmentation_stamped_from_session(self):
        """TD-SEG: company_size/owns_relocation are derived from the linked test_sessions
        row (like campaign/corridor) and written into the survey_responses INSERT, so
        segment + survey answers sit on one queryable row — no join needed."""
        db = MagicMock()
        db.engine.begin.return_value.__enter__.return_value.execute.return_value.first.return_value = None
        ctx = {
            "campaign": "insead-2026", "corridor_id": "FR_NO",
            "company_size": "50-500", "owns_relocation": "run_it",
        }
        with patch.dict(os.environ, _ENABLED, clear=False), \
                patch("backend.app.routers.test_drive.db", db), \
                patch("backend.app.routers.test_drive._session_context", return_value=ctx):
            resp = self.client.post("/api/test-drive/survey", json=_body(trust_intent="yes"))
        self.assertEqual(resp.status_code, 200, resp.text)
        conn = db.engine.begin.return_value.__enter__.return_value
        survey_calls = [c for c in conn.execute.call_args_list if "INSERT INTO survey_responses" in str(c.args[0])]
        self.assertEqual(len(survey_calls), 1)
        self.assertIn("company_size", str(survey_calls[0].args[0]))
        bound = survey_calls[0].args[1]
        self.assertEqual(bound["company_size"], "50-500")
        self.assertEqual(bound["owns_relocation"], "run_it")
        self.assertEqual(bound["corridor_id"], "FR_NO")
        self.assertEqual(bound["trust_intent"], "yes")
