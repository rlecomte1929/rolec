...
import {
  testDriveSurveyContent as c,
  CALENDAR_URL,
  isCalendarUrlConfigured,
} from './testDriveSurveyContent';
...
// TD-EEA (INSEAD cohort): permit-relevance probe — EEA free movement vs Employment Permit.
type PermitRelevance = '' | 'yes' | 'not_yet' | 'no';
...
interface SurveyForm {
  ...
  q3_why: string;
  permit_relevance: PermitRelevance;
  // TD-QA batch 0719 (#5): q4_change dropped from the form — its job ("one thing") now
  // lives in trust_intent_why ("One thing that would make it a yes.").
...
export const TestDriveSurveyPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: SurveyInput = {
      ...
      permit_relevance: form.permit_relevance || undefined,
      ...
      <PublicLayout>
        <Section spacing="lg" background="transparent">
          <div className="mx-auto max-w-xl text-center" role="status" aria-live="polite">
            ...
            {/* #2 Finish → demo bridge. Renders only once CALENDAR_URL holds a real link
                (isCalendarUrlConfigured) — an unset/placeholder value hides the CTA
                entirely rather than shipping a dead link. */}
            {isCalendarUrlConfigured(CALENDAR_URL) && (
              <div className="mx-auto mt-10 max-w-lg rounded-xl border border-marketing-border bg-marketing-surface p-6">
                <p className="text-marketing-body font-medium text-marketing-primary">
                  {c.demoBridge.prompt}
                </p>
                <a
                  href={CALENDAR_URL}
                  ...
                  className="mt-4 inline-flex items-center justify-center rounded-lg bg-marketing-primary px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-marketing-primary-muted focus:outline-none focus:ring-2 focus:ring-marketing-accent focus:ring-offset-2"
                >
                  {c.demoBridge.cta}
                </a>
              </div>
      ...
      <Section spacing="lg" background="muted">
        <FadeIn>
          ...
          >
            ...
            {/* TD-EEA (INSEAD cohort): Q3b — permit relevance. Sits right after the
                problem-fit relevance question; shown for all corridors (EEA and
                Employment Permit alike). */}
            <div>
              <p className="text-sm font-medium text-marketing-primary">{c.q3b.label}</p>
              <TapGroup
                options={c.q3b.options}
                value={form.permit_relevance}
                onSelect={(v) => set('permit_relevance', v as PermitRelevance)}
              />
            </div>

            {/* TD-QA batch 0719 (#5): the trust block is the buy signal (binary Yes/No)
                and its follow-up free text is the roadmap question. The old Q4 ("change
                one thing") was folded into it. */}
            <div>
