...
Deno.serve(async (req) => {
  ...
  if (!safeEqual(req.headers.get("x-otto-token") ?? "", TOKEN)) {
    return json({ ok: false, error: "unauthorized" }, 401);
  }
  const manifest = p.get("manifest"); if (!manifest) return json({ ok: false, error: "manifest required" }, 400);
  ...
  const dryRun = (p.get("dry_run") ?? "true") !== "false";
  const tf = (p.get("tables") ?? "all").toLowerCase();
  ...
  const limitFiles = parseInt(p.get("limit_files") ?? "0") || 0;
