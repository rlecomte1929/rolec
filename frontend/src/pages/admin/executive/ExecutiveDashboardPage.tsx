  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4" data-testid="exec-kpis">
        <StatCard testId="kpi-companies" label="Companies" value={num(data?.growth, 'companies')} sub={`tenants · ${source(data?.growth, loadFailed)}`} loading={loading} fallback={failFallback} />
        <StatCard testId="kpi-employees" label="Employees" value={num(data?.growth, 'employees')} sub={`seats · ${source(data?.growth, loadFailed)}`} loading={loading} fallback={failFallback} />
        <StatCard testId="kpi-signups" label="Signups" value={num(data?.funnel, 'signups')} sub={`profiles · ${source(data?.funnel, loadFailed)}`} loading={loading} fallback={failFallback} />
        <StatCard testId="kpi-cases" label="Cases" value={num(data?.funnel, 'cases')} sub={`total · ${source(data?.funnel, loadFailed)}`} loading={loading} fallback={failFallback} />
