...
import { getTaskUrgency, sortTasksForTracker } from './taskTrackerSort';
...
export const RelocationTaskTracker: React.FC<RelocationTaskTrackerProps> = ({
  ...
  // Tab filter (slice A2): the tab badges used to be visual-only — clicking
  // "Blocked 11" did nothing. Now they filter the list. 'all' shows everything.
  type TaskFilter = 'all' | 'done' | 'overdue' | 'due_soon' | 'blocked' | 'in_progress';
  ...
  const filteredSorted = useMemo(() => {
    ...
    return sorted.filter((m) => {
      ...
      const urgency = getTaskUrgency(m);
      switch (activeFilter) {
        case 'done':
        ...
        case 'overdue':
          return urgency === 'red';
        case 'due_soon':
          return urgency === 'orange' && status !== 'done' && status !== 'skipped';
        case 'blocked':
  ...
  return (
    <Card id="hr-rel-tracker-root" padding="lg" className="overflow-hidden scroll-mt-4">
      ...
      {/* Filter tabs (slice A2). Click to narrow the list below — 'Total'
          resets to all. Active state is highlighted; counts stay visible
          so HR sees the breakdown without needing to switch tabs. */}
      <div className="flex flex-wrap gap-2 mb-4 text-xs" role="tablist" aria-label="Filter tasks by status">
        {(
          [
            { key: 'all', label: 'Total', count: summary.total, baseClass: 'bg-slate-100 text-slate-800 border-slate-200' },
            { key: 'done', label: 'Done', count: summary.completed, baseClass: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
            { key: 'overdue', label: 'Overdue', count: summary.overdue, baseClass: 'bg-red-50 text-red-700 border-red-200' },
            { key: 'due_soon', label: 'Due ≤7d', count: summary.due_this_week, baseClass: 'bg-amber-50 text-amber-800 border-amber-200' },
            { key: 'blocked', label: 'Blocked', count: summary.blocked, baseClass: 'bg-amber-50 text-amber-800 border-amber-200' },
            { key: 'in_progress', label: 'In progress', count: summary.in_progress, baseClass: 'bg-sky-50 text-sky-700 border-sky-200' },
          ] as const
        ).map((tab) => {
          ...
          return (
            <Button unstyled
              ...
              className={`px-2.5 py-1 rounded-full border text-xs font-medium transition-colors ${
                active ? 'ring-2 ring-offset-1 ring-[#0b2b43] bg-[#0b2b43] text-white border-[#0b2b43]' : tab.baseClass
              }`}
            >
              {tab.label} {tab.count}
            </Button>
      ...
      {/* Slice A1: single-column accordion. Each task expands inline
          beneath itself instead of into a separate right panel — fixes
          the case where clicking a row at the top of the list rendered
          the form below the entire list, off-screen, with no visible
          feedback. */}
      <div className="flex flex-col gap-1.5">
        {filteredSorted.length === 0 && (
          <div className="text-sm text-[#6b7280] py-6 text-center border border-dashed border-[#e2e8f0] rounded-lg">
          ...
          return (
            ...
            >
              <Button unstyled
                ...
                className="flex w-full text-left items-stretch"
              >
                <div className={`w-1.5 flex-shrink-0 ${urgencyStripeClass(u)}`} aria-hidden />
                <div className="flex-1 min-w-0 px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500 w-3 text-center text-xs" aria-hidden>
                      {active ? '▾' : '▸'}
                    </span>
                    <span
                ...
                <div className="border-t border-[#e2e8f0] px-4 pt-3 pb-4 bg-white">
                  ...
                  {selected.actual_date && (
                    <p className="text-xs text-[#6b7280] mb-3">
                      Completed {selected.actual_date.slice(0, 10)}
                    </p>
                  )}
