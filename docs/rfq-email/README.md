...
**Applied in-repo (AIQ-2371) before shipping the pack:** (b) invite ask 2 uses ReloPass `RESPONSE_EXPECTATIONS` (“Give an itemised price, not just a single total”); invite copy also keeps “There is no account to create”; (c) `employee_rfq_sent` section retitled “Suppliers we could not reach”; (d) unknown-route subject fallback `Quote request from a relocating company: {{rfq_ref}}`; (e) `hr_quotes_ready` subject is `Quotes ready: {{employee_full_name}} ({{rfq_ref}})`. (a) is a code change: GET `/api/supplier/rfq` is read-only after submit.
