...
| 2 | **Normalize fails** (bad payload / invalid input) | Provoke `422` from normalize (e.g. document not ready per [`normalization_input`](../../backend/services/normalization_input.py)) | Actionable error payload (`NORMALIZATION_NOT_READY`, issues list); HR banner/error path | Employee unchanged vs scenario 1: **still** no published policy or same fallback; **no** comparison | `policy_normalize_started` → `policy_normalize_failed` (`error_code=NORMALIZATION_NOT_READY`). **No** `policy_normalize_completed` |
