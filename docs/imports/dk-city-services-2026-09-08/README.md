# dk-city-services-2026-09-08 — Denmark city services (draft)
...
- **6 `country_resources` rows** (cities: roskilde; 6 services each) — all
  `status='draft'`, `is_visible_to_end_users=false`. 6 `resource_sources`.
- Append-only verified: Denmark published/visible unchanged (0→0); totals grew by the inserts.
