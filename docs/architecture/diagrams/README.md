...
| 03 | [Tier-snapshot binding](./03-tier-snapshot-binding.png) | One policy per company, with employee-tier values inside it. Cases bind to the published policy snapshot at the employee's tier. Blank tier values return "not configured + escalation" rather than silent fallbacks. |
