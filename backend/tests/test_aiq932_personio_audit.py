"""
AIQ-932 (slice 2) — canonical audit_logs for Personio HRIS connection writes.
...
from backend.app.routers import integrations_personio_settings as personio  # noqa: E402
...
def _patch():
    ...
    audit = MagicMock()
    return patch.multiple(personio, SessionLocal=sl, insert_audit_log=audit), audit, session


class TestPersonioAudit(unittest.TestCase):
    def test_connect_insert_audited(self):
        ctx, audit, session = _patch()
        with ctx:
            personio._audit_personio(USER, "conn-1", ACTION_INSERT, "personio_connected")
        kw = audit.call_args.kwargs
        self.assertEqual(kw["entity_type"], "hris_connection")
        self.assertEqual(kw["entity_id"], "conn-1")
        self.assertEqual(kw["action_type"], ACTION_INSERT)
        self.assertEqual(kw["actor_type"], ACTOR_HUMAN)
        self.assertEqual(kw["actor_id"], "hr-7")
        self.assertEqual(kw["new_value"]["event"], "personio_connected")
        self.assertEqual(kw["new_value"]["provider"], "personio")
        session.commit.assert_called_once()

    def test_disconnect_update_audited(self):
        ctx, audit, _ = _patch()
        with ctx:
            personio._audit_personio(USER, "conn-1", ACTION_UPDATE, "personio_disconnected")
        self.assertEqual(audit.call_args.kwargs["action_type"], ACTION_UPDATE)
        self.assertEqual(audit.call_args.kwargs["new_value"]["event"], "personio_disconnected")

    def test_audit_is_failsoft(self):
        ctx, audit, _ = _patch()
        audit.side_effect = RuntimeError("db down")
        with ctx:
            personio._audit_personio(USER, "conn-1", ACTION_UPDATE, "personio_field_mappings_updated")
