...
The real implementation calls OpenAI through the shared ``llm_client``
wrapper. Tests patch ``catalog_scraper._call_llm`` so we never hit the
network and never need an API key — the patched value is the parsed JSON
payload the wrapper would have returned.
"""
...
def _patch_llm(payload: dict):
    """Patch the wrapper seam so populate_destination_catalog receives the
    given parsed JSON payload instead of calling OpenAI."""
...
class CatalogScraperTests(unittest.TestCase):
    ...
    def test_no_op_when_disabled(self) -> None:
        with mock.patch.dict(os.environ, {"CATALOG_SCRAPER_ENABLED": "0"}):
            with _patch_llm({"vendors": [{"name": "X"}]}) as m:
    ...
    def test_llm_failure_returns_empty(self) -> None:
        # The wrapper raises ValueError on non-JSON output; populate swallows
        # it and returns [] so case creation never fails on a bad LLM reply.
        with mock.patch.object(
            catalog_scraper, "_call_llm", side_effect=ValueError("non-JSON")
        ):
            result = catalog_scraper.populate_destination_catalog(
                category="movers", destination_city="Tokyo",
            )
    ...
    def test_short_circuit_skips_llm_when_already_populated(self) -> None:
        ...
        payload = {"vendors": [{"name": "Should Not Insert", "summary": "x", "website": None,
                                "strengths": [], "notes": None}]}
        with _patch_llm(payload) as m:
            result = catalog_scraper.populate_destination_catalog(
                category="movers", destination_city="Tokyo",
            )
        self.assertEqual(result, [])
