...
def test_rerunning_stages_nothing_new(engine, candidates):
    ...
    This test used to assert `dup == 29` — it encoded the append-a-duplicate-set behaviour AS
    ...
    assert pending == 29, "the second run must not add a single new pending candidate"
...
def test_promotion_lands_everything_unvetted(full_engine, candidates):
    ...
    assert promoted == 29, "every candidate is accounted for, merged or new"
    with full_engine.begin() as conn:
    ...
    assert (pending, approved) == (29, 0), "nothing may arrive pre-approved"
...
def test_a_company_in_two_corridors_is_one_supplier_with_two_capabilities(
    ...
    So 29 candidates become 27 suppliers and 29 capabilities."""
    from backend.imports.suppliers.executor import promote
...
def test_the_registry_evidence_survives_promotion(full_engine, candidates):
    ...
    # 27, not 29: the unique claim index is (supplier_id, body, scheme), so a company in two
...
def test_promotion_is_idempotent(full_engine, candidates):
    ...
    assert again == 0
    with full_engine.begin() as conn:
        assert conn.execute(text("SELECT count(*) FROM suppliers")).scalar_one() == 27
...
def test_staging_twice_writes_nothing_the_second_time(full_engine, candidates):
    ...
    pass re-staged all 29 as 'duplicate' under 8 fresh runs — 58 candidates and 16 runs where
    there should have been 29 and 8. The directory was unharmed (the supplier name index and
...
def test_a_genuinely_new_row_still_stages_after_a_first_run(full_engine, candidates):
    ...
    assert (total, landed) == (30, 1)  # the 29 already staged, plus the newcomer
