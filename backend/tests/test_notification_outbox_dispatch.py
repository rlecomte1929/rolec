"""[AIQ-1610] The notification_outbox consumer actually sends pending rows.
...
These tests pin the consumer's contract:
  ...
  * a pending row is delivered via Resend and marked 'sent'
...
def test_pending_row_is_sent_and_marked_sent(monkeypatch):
    ...
    assert summary == {"pending": 1, "sent": 1, "logged": 0, "failed": 0}
...
def test_no_key_is_counted_logged_but_row_moves_terminal(monkeypatch):
    ...
    assert summary == {"pending": 1, "sent": 0, "logged": 1, "failed": 0}
