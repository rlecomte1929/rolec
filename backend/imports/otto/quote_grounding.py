...
TextResolver = Callable[[str], Tuple[Optional[str], str]]
...
def default_normalise(text: str) -> str:
    ...
    The integrated path injects ``backend.app.services.fact_evidence.normalise``
    (which does not case-fold). Tests use this default unless they pass another.
    """
    out = (text or "").lower()
...
def grounding_text_for(url: str, grounding_dir: Path) -> Optional[str]:
    ...
    present. If the index is absent (or has no entry for this URL), fall back to
    ``<slug(url)>.txt``.
    """
    ...
    if index_path.is_file():
        try:
            ...
            mapping = {}
        rel = mapping.get(url) if isinstance(mapping, dict) else None
        if rel:
            path = grounding_dir / rel
            if path.is_file():
...
def load_text_index(cache_dir: Path) -> Dict[str, str]:
    """Load URL -> page text from a cache/grounding dir (``index.json`` + files)."""
...
def check_quotes(
    ...
    *,
    ...
    for rec in records:
        ...
        quote = None if raw_quote is None else str(raw_quote)
        if quote is None or not quote.strip():
            verdicts.append(
                QuoteVerdict(
                    dedupe_key=key,
                    source_url=url,
                    evidence_quote=None if quote is None or not quote.strip() else quote,
                    status="no_quote",
                    grounded_by=None,
                )
            )
        ...
        if isinstance(resolved, tuple):
            page_text, tag = resolved[0], (resolved[1] if len(resolved) > 1 else "http_fetch")
        else:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.fetched_cache is not None:
        ...
        # Also resolve via grounding_text_for semantics so URL-keyed index works
        # even when tests only drop files (index maps URL -> file).
        if (args.fetched_cache / "index.json").is_file():
            mapping = json.loads((args.fetched_cache / "index.json").read_text(encoding="utf-8"))
            for url in mapping:
                text = grounding_text_for(url, args.fetched_cache)
                if text is not None:
                    fetched[url] = text
